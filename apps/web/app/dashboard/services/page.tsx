...
export default async function ServicesPage({
  ...
  const servicesWithImageCount = (services ?? []).filter((service) =>
    Boolean(service.image_path),
  ).length;
  const averagePrice =
    servicesCount === 0
      ? 0
  ...
  const groupedServices = (services ?? []).reduce<
    Record<string, NonNullable<typeof services>>
  >((groups, service) => {
  ...
  const largestCategory = Object.entries(groupedServices).sort(
  ...
  return (
    <div className="page-grid workspace-page services-page">
      <DashboardWorkspaceHero
        eyebrow="Serviços"
        title="Catálogo do salão"
        description="Organize os serviços que entram no app, no catálogo e na agenda da equipe."
        highlight={{
        ...
        actions={
          <>
            ...
            <a href="#new-service" className="secondary-button">
              Novo serviço
            </a>
          </>
        }
          ...
          <>
            ...
            <h3>
              {servicesWithImageCount > 0
                ? "O catálogo já tem base visual."
                : "Vale começar pelas fotos de maior ticket."}
            </h3>
            <p>
              {servicesWithImageCount > 0
                ? `${servicesWithImageCount} serviço${servicesWithImageCount === 1 ? "" : "s"} já têm imagem pronta para o app.`
                : "Serviços sem foto ainda funcionam, mas perdem força visual no app."}
            </p>
          </>
        }
      />
      ...
      <div className="two-column-grid">
        <section id="service-catalog" className="card content-card">
          <div className="section-heading">
            <div>
              <h2>Serviços cadastrados</h2>
              <p className="muted">Busque, filtre e ajuste o catálogo.</p>
            </div>
          </div>
          ...
          <form method="get" className="services-toolbar">
            ...
            <div className="services-toolbar__actions">
              ...
              <span className="muted service-results-meta">
                {services?.length ?? 0}{" "}
                {(services?.length ?? 0) === 1 ? "resultado" : "resultados"}
              </span>
            </div>
          </form>
          ...
          <div className="row-list" style={{ marginTop: 16 }}>
            {!services?.length ? (
              <EmptyStateCard
                eyebrow={
                  hasActiveFilters ? "Nenhum resultado" : "Comece por aqui"
                }
                title={
                  hasActiveFilters
                      ...
                      >
                        <div className="service-editor-grid">
                            ...
                            {service.image_path ? (
                              <Image
                                src={
                                  supabase.storage
                                    .from("salon-assets")
                                    .getPublicUrl(service.image_path).data
                                    .publicUrl
                                }
                                alt={service.name}
                                width={240}
                              ...
                              <div className="service-preview-placeholder">
                                ...
                                <span className="muted">
                                  Adicione uma imagem para valorizar a vitrine
                                  no app do cliente.
                                </span>
                              </div>
                            )}
                          ...
                          <div className="list-row__content">
                            <form
                              action={updateServiceCatalogAction}
                              className="form-grid"
                            >
                              <input
                                type="hidden"
                              ...
                              <div className="split-grid">
                                <div className="field">
                                  <label
                                    htmlFor={`service-order-${service.id}`}
                                  >
                                    Ordem de exibição
                                  </label>
                                  <input
                            ...
                            <div className="service-editor-footer">
                              <ConfirmServiceDeleteButton
                                serviceId={service.id}
                                serviceName={service.name}
                              />
                              <small className="muted service-delete-note">
                                A exclusão é bloqueada quando o serviço já
                                estiver em agendamentos ou vinculado ao feed.
                              </small>
                            </div>
                          </div>
                        </div>
        ...
        <section id="new-service" className="card content-card form-panel">
          <div className="section-heading">
            <div>
              <h2>Novo serviço</h2>
              <p className="muted">
                Cadastre um atendimento para entrar no app.
              </p>
            </div>
          </div>
          <form action={createServiceAction} className="form-grid">
            ...
            <small className="muted">
              Depois você ajusta equipe e ordem de exibição quando precisar.
            </small>
          </form>
